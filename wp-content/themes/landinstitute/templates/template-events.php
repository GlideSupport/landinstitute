...
document.getElementById('load-more-events').addEventListener('click', function (e) {
  ...
  fetch('<?php echo admin_url("admin-ajax.php"); ?>?action=load_more_events&page=' + page)
    .then(res => res.text())
    .then(html => {
...
jQuery(document).ready(function () {
	...
	jQuery('.event-calendar-row').each(function (i, v) {
		jQuery(this).find('.calendar-column').removeClass('ct-first-row');
		jQuery(this).find('.calendar-column').removeClass('ct-second-row');
		jQuery(this).find('.calendar-column').removeClass('ct-third-row');
		jQuery(this).find('.calendar-column').removeClass('ct-forth-row');
		jQuery(this).find('.calendar-column').removeClass('ct-fifth-row');

		jQuery(this).find('.calendar-column').removeClass('ct-first-col');
		jQuery(this).find('.calendar-column').removeClass('ct-second-col');
		jQuery(this).find('.calendar-column').removeClass('ct-third-col');
		jQuery(this).find('.calendar-column').removeClass('ct-forth-col');
		jQuery(this).find('.calendar-column').removeClass('ct-fifth-col');
		jQuery(this).find('.calendar-column').removeClass('ct-sixth-col');
		jQuery(this).find('.calendar-column').removeClass('ct-seventh-col');
		
		const col_length = jQuery(this).find('.calendar-column').length;
		const month = jQuery(this).find('.calendar-month .heading-5').text();
		const day = new Date(`01 ${month}`).getDay();
		...
		jQuery(this).find('.calendar-column').each(function (ii, vv) {
			var row_class = '';
			var col_class = '';
			if (ii < 7) {
				var row_class = 'ct-first-row';
				var cols = 7;
			} else if (ii >= 7 && ii < 14) {
				var row_class = 'ct-second-row';
				var cols = 7;
			} else if (ii >= 14 && ii < 21) {
				var row_class = 'ct-third-row';
				var cols = 7;
			} else if (ii >= 21 && ii < 28) {
				var row_class = 'ct-forth-row';
				var cols = 7;
			} else if (ii >= 28 && ii < 31) {
				var row_class = 'ct-firth-row';
				var cols = col_length - 28;
			} else if (ii >= 31) {
				var row_class = 'ct-firth-row';
				var cols = col_length - 28;
			}
			const arr_col0 = [1, 8, 15, 22, 29];
			const arr_col1 = [2, 9, 16, 23, 30];
			const arr_col2 = [3, 10, 17, 24, 31];
			const arr_col3 = [4, 11, 18, 25, 32];
			const arr_col4 = [5, 12, 19, 26, 33];
			const arr_col5 = [6, 13, 20, 27, 34];
			const arr_col6 = [7, 14, 21, 28, 35];
			// console.log(parseInt(ii) + 1);
			// console.log(jQuery.inArray(parseInt(ii) + 1, arr_col0));
			let col_number = 0;
			if (jQuery.inArray(parseInt(ii) + 1, arr_col0) !== -1) {
				var col_class = 'ct-first-col';
				col_number = 1;
			} else if (jQuery.inArray(parseInt(ii) + 1, arr_col1) !== -1) {
				var col_class = 'ct-second-col';
				col_number = 2;
			} else if (jQuery.inArray(parseInt(ii) + 1, arr_col2) !== -1) {
				var col_class = 'ct-third-col';
				col_number = 3;
			} else if (jQuery.inArray(parseInt(ii) + 1, arr_col3) !== -1) {
				var col_class = 'ct-forth-col';
				col_number = 4;
			} else if (jQuery.inArray(parseInt(ii) + 1, arr_col4) !== -1) {
				var col_class = 'ct-fifth-col';
				col_number = 5;
			} else if (jQuery.inArray(parseInt(ii) + 1, arr_col5) !== -1) {
				var col_class = 'ct-sixth-col';
				col_number = 6;
			} else if (jQuery.inArray(parseInt(ii) + 1, arr_col6) !== -1) {
				var col_class = 'ct-seventh-col';
				col_number = 7;
			}
			// console.log(col_class);
			jQuery(this).find('.calendar-event').attr('data-position', col_number);
			jQuery(this).find('.calendar-event').attr('data-cols', cols);
			jQuery(this).find('.calendar-event').addClass(col_class + '-item');

			// jQuery(this).find('.calendar-event').each(function () {

			// })
			jQuery(this).addClass(row_class);
			jQuery(this).addClass(col_class);
		});
	});
});
