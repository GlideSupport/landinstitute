...
document.addEventListener( 'DOMContentLoaded', function() {
	...
	// CTA Slider JS start
		...
		ctaSliderWrappers.forEach((wrapper, index) => {
			...
			// Skip if container or slides are missing
			...
			function updateCounter(swiperInstance) {
				...
				const realSlides = swiperInstance.slides.filter(slide =>
			...
			function updateArrowPosition() {
				const allSlideContents = wrapper.querySelectorAll('.cta-slider-lft-block .cl-left .slide-content');
				let maxHeight = 0;
	...
	// Testimonial Traditional js start
		...
		sliderWrappers.forEach((wrapper, index) => {
			...
			const wrapperClass = [...wrapper.classList].find(cls => cls.startsWith("tslider_"));

			// Skip if container or slides are missing or less than 1 slide
			if (!swiperContainer || !swiperWrapper || !slides || slides.length < 1 || !wrapperClass) return;
