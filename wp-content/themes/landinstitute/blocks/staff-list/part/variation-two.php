...
<div class="staff-list tabbed-content-block has-border-bottom">
	...
	<?php if (!empty($li_sl_staff_selector)) : ?>
		<div class="tabbed-block-content">
			<div class="tabs-listing">
				<?php
				$terms = get_terms(['taxonomy' => 'staff-category', 'hide_empty' => true, 'parent' => 0, 'object_ids' => $li_sl_staff_selector,]);
					...
					if ($all_query->have_posts()) :
						while ($all_query->have_posts()) : $all_query->the_post();
							...
							foreach ($staff_terms as $term) {
								if ($term->parent === 0) {
									$parent_term = $term;
									break; // stop at the first parent term
								}
							}
							$staff_bg_color = 'bg-sunflower-yellow'; // Default for "All" tab
							if ($parent_term) {
								$term_color = get_field('li_globel_bg_color_options', 'term_' . $parent_term->term_id);
								if (!empty($term_color)) {
									$staff_bg_color = $term_color;
								}
							}
							echo '
