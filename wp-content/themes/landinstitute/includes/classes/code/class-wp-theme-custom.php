...
class WP_Theme_Custom extends \Boilerplate
{
	 ...
	 * @param string $classes are the string of classes of acf block title.
	 *
	 * @return string
	 */

	public static function headline_check($object)
	{
		if (!empty($object)):

			$title_text = '';

			// Loop through the array to find the keys ending with '_title' 
			foreach ($object as $key => $value) {
				if (preg_match('/_title$/', $key)) {
					$title_text = $value;
				}
			}
	...
	/**
	 * Helper function that builds button from ACF Block title object
	 *
	 * @param object $object is a acf block title object.
	 * @param string $classes are the string of classes of acf block title.
	 *
	...
	public static function headline($object, $classes = '')
	{
		if (!empty($object)):
			...
			$title_text = '';
			$title_tag = 'h2'; // Default tag if not found

			// Loop through the array to find the keys ending with '_title' and '_title_tag'
			foreach ($object as $key => $value) {
				if (preg_match('/_title$/', $key)) {
					$title_text = $value;
				} elseif (preg_match('/_title_tag$/', $key)) {
					$title_tag = $value;
				}
			}
	 ...
	 * @return string
	 */
	public static function button($object, $classes = '', $icon = '', $path = 'path', $icon_position = 'left')
	{
		if (is_array($object) && isset($object['url']) && !empty($object['url'])) {
			$title = $object['title'] ? $object['title'] : 'Learn More';
			if ($path == 'path' && $icon != '') {
				$icon = '<img src="' . $icon . '" alt="' . $title . '">';
			}
			$link = '';
			$link = "<a href='" . esc_url($object['url']) . "'";
			$link .= " title='" . esc_html($title) . "'";
			$link .= " role='" . 'button' . "'";
			$link .= " aria-label='" . esc_html($title) . "'";
			$link .= " data-text='" . esc_html($title) . "'";
	 ...
	 *
	 * @param mixed $input Input to decode. Can be any type.
	 * @return string Sanitized and decoded string.
	 */
	public static function html_entity_remove($input)
	{
		// Ensure input is a string, default to an empty string if null or invalid.
		$string = $input ?? '';  // Null coalescing operator to handle null values.

		// Decode HTML entities and sanitize text.
		return sanitize_text_field(wp_kses_post(html_entity_decode($string, ENT_QUOTES | ENT_HTML5, 'UTF-8')));
	}
	...
	public static function nav_fallback()
	{
		...
		if (is_user_logged_in()) {
			?>
			<ul>
				<li> <?php echo esc_html__('Go to admin area to create navigation menu', 'land_institute'); ?></li>
			</ul>
			<?php
		}
	}
	...
	public static function the_social_icons($social_icons, $output_type = true)
	{
		if ($social_icons) {
			foreach ($social_icons as $social_icon) {
				...
				$svg_hover_code = self::get_hover_social_svg($acf_fc_layout);
				$svg_link = self::get_social_link($acf_fc_layout);
				$svg_hover_link = self::get_hover_social_link($acf_fc_layout);
				...
				if ($social_link) {
					...
					if ($output_type) {
						...
						echo '<img class="hover-social-icon" height="20" width="20" src="' . esc_url($svg_hover_link) . '" title="' . esc_attr($title) . '" alt="' . esc_attr($title) . '">';
					}
					echo '</a>';
				}
			}
	 ...
	 *
	 * @param string $key Social icon key.
	 * @return string SVG content or empty string if not found.
	 */
	public static function get_social_svg($key)
	{
		$file_path = get_template_directory() . '/assets/build/images/social-icons/' . $key . '.svg';
	...
	/**
	 * Output hover Social icon SVG.
	 *
	 * @param string $key Social icon key.
	 * @return string SVG content or empty string if not found.
	 */
	public static function get_hover_social_svg($key)
	{
		$file_path = get_template_directory() . '/assets/build/images/social-icons/' . $key . '-hover.svg';
	 ...
	 *
	 * @param string $key Social icon key.
	 * @return string URL of the social icon or empty string if the file doesn't exist.
	 */
	...
	/**
	 * Output hover Social icon link.
	 *
	 * @param string $key Social icon key.
	 * @return string URL of the hover social icon or empty string if the file doesn't exist.
	 */
	public static function get_hover_social_link($key)
	{
		$file_path = get_template_directory() . '/assets/build/images/social-icons/' . $key . '-hover.svg';

		if (file_exists($file_path)) {
			return get_template_directory_uri() . '/assets/build/images/social-icons/' . $key . '-hover.svg';
		}
	 ...
	 * @param string|null $local_vars Local variable name.
	 * @param array       $data Local variable data.
	...
	public static function enqueue_script($path, $dependencies = array('jquery'), $local_vars = null, $data = array(), $args = array())
	{
		...
		$version = file_exists($file_path) ? filemtime($file_path) : false; // Check if file exists for versioning
	 ...
	 * @param string      $path         Path of the file.
	 * @param array       $dependencies File dependencies.
	 * @param string|null $local_vars   Local variable name for localization.
	 * @param array       $data         Data for localization.
	 * @param array       $args         Additional arguments for script registration.
	 *
	 * @return string The registered script handle.
	 */
	public static function register_script($path, $dependencies = array('jquery'), $local_vars = null, $data = array(), $args = array())
	{
		...
		$name = 'wp-theme-' . (explode('.', basename($path))[0] ?? basename($path));
	...
	public static function enqueue_style($path)
	{
		...
		if (file_exists($file_path)) {
			...
			// Default version or fallback to prevent errors
			$version = false;
		}
