...
class WP_Theme_Acf {
	...
	public function __construct() {
		...
		if ( function_exists( 'acf_add_options_page' ) ) {
			$option_page = acf_add_options_page(
				array(
					'page_title' => __( 'Theme Options', 'land_institute' ),
					'menu_title' => __( 'Theme Options', 'land_institute' ),
	...
	public function blocks_category( $categories ) {
		$custom_block = array(
			'slug'  => 'theme-blocks',
			'title' => __( 'Theme Blocks', 'land_institute' ),
			'icon'  => 'theme-blocks',
		);
...
jQuery(document).ready(function() {
    setTimeout(() => {
        jQuery('<?php echo esc_html( $field_class ); ?>').find('iframe').css('height',
            '<?php echo esc_html( $custom_acf_wysiwyg_height ); ?>px');
    }, 1000);
});
</script>
...
<?php
	...
	public function wysiwyg_render_field_settings( $field ) {
		acf_render_field_setting(
			$field,
			array(
				'label'        => __( 'Height of Editor', 'land_institute' ),
				'instructions' => __( 'Height of Editor after Init', 'land_institute' ),
				'name'         => 'wysiwyg_height',
				'type'         => 'number',
			)
		);
