...
if ( ! empty( $_SERVER['SCRIPT_FILENAME'] ) && 'comments.php' === basename( esc_url_raw( wp_unslash( $_SERVER['SCRIPT_FILENAME'] ) ) ) ) { /*1*/
		die( esc_html__( 'Please do not load this page directly. Thanks!', 'land_institute' ) );
}
if ( post_password_required() ) {
	esc_html__( 'This post is password protected. Enter the password to view comments.', 'land_institute' );
	return;
}
	...
	<div id="comments" class="comments-area">
		...
		<h3 class="section-title" id="comments">
			...
			if ( '1' === $bst_var_comment_count ) {
				printf(
					/* translators: 1: title. */
					esc_html__( 'One Response', 'land_institute' ),
					'<span>' . esc_html( get_the_title() ) . '</span>'
				);
			} else {
				printf(
					/* translators: 1: comment count number, 2: title. */
					esc_html( _nx( '%1$s Response', '%1$s Response', $theme_comment_count, 'comments title', 'land_institute' ) ),
		...
		<?php } else { // this is displayed if there are no comments so far. ?>
			...
			if ( ! comments_open() ) {
				?>
					<p class="no-comments"><?php esc_html__( 'Comments are closed.', 'land_institute' ); ?></p>
				<?php
			}
